...
{
    [TestFixture]
    public class GetTopHealingOnPage_Should
    {
        [TestCase(1)]
        [TestCase(3)]
        [TestCase(42)]
        ...
        public void InvokeIOutputPerSecondViewModelDataProvider_GetTopHealingOnPageInDescendingOrderAsManyTimesAsRequestedPages(int pageNumber)
        {
            ...
            // Assert
            outputPerSecondViewModelDataProvider.Verify(p => p.GetTopHealingOnPageInDescendingOrder(It.IsAny<int>(), It.IsAny<int>()), Times.Exactly(pageNumber));
        }
